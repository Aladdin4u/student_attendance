...
<aside id="logo-sidebar" aria-label="Sidebar" class="min-h-screen">
    <div class="h-[90vh] px-3 pb-4 dark:bg-gray-800">
        <ul class="h-[85vh] flex flex-col space-y-2 font-medium bg-white overflow-y-auto">
            <li>
                <a href="{{$dashboardLink}}" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-[#15ACD9] hover:text-white group {{ $isActive ? 'bg-[#15ACD9] text-white' : ''}}">
                    <svg class="w-6 h-6 text-gray-500 transition duration-75 group-hover:text-white {{ $isActive ? 'bg-[#15ACD9] text-white' : ''}}" fill="none" stroke="currentColor" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
