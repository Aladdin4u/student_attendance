<li>
    <a href="/users/{{auth()->user()->id}}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-sky-100 hover:text-sky-500 dark:hover:bg-sky-700 group {{ request()->is('students') ? 'bg-sky-100 text-sky-500' : ''}}">
...
<li x-data="{ 
    ...
    <ul class="space-y-2 font-medium pl-10" x-show="open">
        ...
        <li><a href="/attendances/manage" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:text-sky-500 hover:bg-sky-100 {{ request()->is('attendances/manage') ? 'bg-sky-100 text-sky-500' : ''}}">Overall Attendance</a></li>
    </ul>
</li>