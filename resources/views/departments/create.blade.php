        ...
        <form method="POST" action="/departments" class="space-y-4">
            ...
            <x-row>
                <div class="basis-1/2">
                    ...
                    <x-input type="text" name="name" placeholder="Department Code" value="{{old('name')}}" />
                    @error('name')
                    <x-alert>{{$message}}</x-alert>
                    @enderror
                </div>
                <div class="basis-1/2">
                    <x-label for="faculty_id">Faculty</x-label>
                    <div>
                        <x-select name="faculty_id" id="faculty_id">
                            ...
                            <x-option value="{{$faculty->id}}" class="hover:bg-sky-100">{{$faculty->name}}</x-option>
                            @endforeach
                            @endunless
                        </x-select>
                    </div>
